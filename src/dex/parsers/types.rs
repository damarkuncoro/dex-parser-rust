...
pub fn parse(buffer: &[u8], header: &RawHeader, strings: &[String], endian: Endian) -> Result<Vec<String>, DexError> {
    ...
    for i in 0..header.type_ids_size {
        ...
        let descriptor = strings.get(type_id.descriptor_idx as usize)
            .ok_or_else(|| DexError::InvalidIndex(format!("Type descriptor_idx {}", type_id.descriptor_idx)))?;
