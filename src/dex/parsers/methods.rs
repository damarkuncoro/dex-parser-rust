...
pub fn parse(buffer: &[u8], header: &RawHeader, strings: &[String], types: &[String], endian: Endian) -> Result<Vec<String>, DexError> {
    ...
    for i in 0..header.method_ids_size {
        ...
        let class_name = types.get(method_id.class_idx as usize)
            .ok_or_else(|| DexError::InvalidIndex(format!("Method class_idx {}", method_id.class_idx)))?;
        let method_name = strings.get(method_id.name_idx as usize)
            .ok_or_else(|| DexError::InvalidIndex(format!("Method name_idx {}", method_id.name_idx)))?;
