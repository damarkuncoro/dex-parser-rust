...
pub fn parse(buffer: &[u8], header: &RawHeader, strings: &[String], types: &[String], endian: Endian) -> Result<Vec<Field>, DexError> {
    ...
    for i in 0..header.field_ids_size {
        ...
        fields.push(Field {
            class: types.get(raw.class_idx as usize).cloned().unwrap_or_default(),
            type_name: types.get(raw.type_idx as usize).cloned().unwrap_or_default(),
            name: strings.get(raw.name_idx as usize).cloned().unwrap_or_default(),
        });
    }
    Ok(fields)
}
