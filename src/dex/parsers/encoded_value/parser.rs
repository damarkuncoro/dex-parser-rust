...
pub fn parse_encoded_value<'a, R: DexResolver<'a>>(
    ...
    let value = match value_type {
        value_types::BYTE => {
            ...
            let val = utils::read_int(buffer, &mut curr, value_arg + 1)? as i8;
            reader.read_bytes(curr - start)?;
            EncodedValue::Byte(val)
        },
        value_types::SHORT => {
            let start = reader.position();
            let val = utils::read_int(buffer, &mut curr, value_arg + 1)? as i16;
            reader.read_bytes(curr - start)?;
            EncodedValue::Short(val)
        },
        value_types::CHAR => {
            let start = reader.position();
            let val = utils::read_uint(buffer, &mut curr, value_arg + 1)? as u16;
            reader.read_bytes(curr - start)?;
            EncodedValue::Char(val)
        },
        value_types::INT => {
            let start = reader.position();
            let val = utils::read_int(buffer, &mut curr, value_arg + 1)? as i32;
            reader.read_bytes(curr - start)?;
            EncodedValue::Int(val)
        },
        value_types::LONG => {
            let start = reader.position();
            let val = utils::read_int(buffer, &mut curr, value_arg + 1)?;
            reader.read_bytes(curr - start)?;
            EncodedValue::Long(val)
        },
        value_types::FLOAT => {
            let start = reader.position();
            let val = utils::read_float(buffer, &mut curr, value_arg + 1)?;
            reader.read_bytes(curr - start)?;
            EncodedValue::Float(val)
        },
        value_types::DOUBLE => {
            let start = reader.position();
            let val = utils::read_double(buffer, &mut curr, value_arg + 1)?;
            reader.read_bytes(curr - start)?;
            EncodedValue::Double(val)
        },
        value_types::STRING => {
            let start = reader.position();
            let idx = utils::read_uint(buffer, &mut curr, value_arg + 1)? as u32;
            reader.read_bytes(curr - start)?;
        ...
        value_types::ARRAY => {
            EncodedValue::Array(parse_encoded_array(reader, resolver)?)
        },
