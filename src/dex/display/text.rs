use crate::dex::models::{Dex, Class, EncodedMethod, EncodedField};
...
impl DexDumpPrinter {
    fn print_header(&self, dex: &Dex) {
        ...
        println!("signature           : {}...{}", &sig_hex[..4], &sig_hex[sig_hex.len()-4..]);
        println!("file_size           : {}", dex.header.file_size);
        println!("header_size         : {}", dex.header.header_size);
        println!("link_size           : {}", dex.header.link_size);
        println!("link_off            : {} (0x{:06x})", dex.header.link_off, dex.header.link_off);
        println!("string_ids_size     : {}", dex.strings.len());
        println!("string_ids_off      : {} (0x{:06x})", dex.header.string_ids_off, dex.header.string_ids_off);
        println!("type_ids_size       : {}", dex.types.len());
        println!("type_ids_off        : {} (0x{:06x})", dex.header.type_ids_off, dex.header.type_ids_off);
        println!("proto_ids_size      : {}", dex.protos.len());
        println!("proto_ids_off       : {} (0x{:06x})", dex.header.proto_ids_off, dex.header.proto_ids_off);
        println!("field_ids_size      : {}", dex.fields.len());
        println!("field_ids_off       : {} (0x{:06x})", dex.header.field_ids_off, dex.header.field_ids_off);
        println!("method_ids_size     : {}", dex.methods.len());
        println!("method_ids_off      : {} (0x{:06x})", dex.header.method_ids_off, dex.header.method_ids_off);
        println!("class_defs_size     : {}", dex.classes.len());
        println!("class_defs_off      : {} (0x{:06x})", dex.header.class_defs_off, dex.header.class_defs_off);
        println!("data_size           : {}", dex.header.data_size);
        println!("data_off            : {} (0x{:06x})", dex.header.data_off, dex.header.data_off);
    }

    fn print_class(&self, _dex: &Dex, index: usize, class: &Class) {
        println!("\nClass #{:<12} -", index);
        println!("  Class descriptor  : '{}'", class.name);
        println!("  Access flags      : 0x{:04x} ({})", class.access_flags, class.access_flags_text);
        ...
        let source_file = class.source_file.as_ref().map(|s| format!("'{}'", s)).unwrap_or_else(|| "unknown".to_string());
        println!("  source_file_idx   : {} ({})", class.source_file_idx, source_file);
    }
    ...
    fn print_field(&self, idx: usize, field: &EncodedField, class_name: &str) {
        ...
        println!("      access        : 0x{:04x} ({})", field.access_flags, field.access_flags_text);
    }
    ...
    fn print_method(&self, idx: usize, method: &EncodedMethod, class_name: &str) {
        ...
        println!("      access        : 0x{:04x} ({})", method.access_flags, method.access_flags_text);
        if let Some(code) = &method.code {
            println!("      code          -");
            println!("      registers     : {}", code.registers_size);
            println!("      ins           : {}", code.ins_size);
            println!("      outs          : {}", code.outs_size);
            println!("      insns size    : {} 16-bit code units", code.insns_size);

            for ins in &code.instructions {
                println!("      0x{:04x}: {} {}", ins.offset, ins.name, ins.description);
            }
            ...
            if code.catches.is_empty() {
                ...
                for catch in &code.catches {
                    println!("        0x{:04x} - 0x{:04x}", catch.start_addr, catch.end_addr);
